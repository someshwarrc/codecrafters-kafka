...
public static class StreamExtensions
{
    ...
    public static async Task HandleApiVersionRequest(this NetworkStream stream, int correlation_id, int api_key, int api_version) {
        ...
        client_software_name - COMPACT_STRING
            client_software_name_length : 1 byte - Assuming length < 256 (0xff)
            client_software_name        : (client_software_name_length - 1) bytes
        client_software_version - COMPACT_STRING
            client_software_ver_length  : 1 byte - Assuming length < 256 (0xff)
            client_software_ver         : (client_software_ver_length - 1) bytes
        tag_buffer                  : 1 byte
        */
    ...
    public static async Task ParseRequest(this NetworkStream stream) {
        ...
        switch(api_key) {
            ...
            case 75: await stream.HandleDescribeTopicPartitionRequest(correlation_id);
                break;
            default: await stream.HandleApiVersionRequest(correlation_id, api_key, api_version);
                break;
        }
